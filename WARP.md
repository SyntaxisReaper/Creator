...
- Contact form: Submits to `/api/contact`, which is implemented as a serverless function in `api/contact.js` and sends mail through Nodemailer.
- Required env vars for deployment: `EMAIL_USER`, `EMAIL_PASS`, and optionally `EMAIL_TO` and `SMTP_HOST` / `SMTP_PORT`.
...
- script.js
  - Progressive UI behavior using browser APIs (no dependencies):
    ...
    - Contact form handler POSTs JSON to `/api/contact` and shows success/error status in the form.

Deployment

- Static hosting still works for the UI, but the contact form needs a host that runs serverless functions, such as Vercel or Netlify with function support.
